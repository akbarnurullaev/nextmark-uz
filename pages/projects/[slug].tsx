  ...
  return (
    ...
    >
      <g filter="url(#filter0_f_153_132)">
        <path
          d="M410.074 173.005C322.074 219.805 331.741 279.505 347.574 303.505C245.574 372.005 327.074 482.005 410.074 503.005C493.074 524.005 520.074 471.505 586.574 386.505C653.074 301.505 669.574 233.005 607.074 189.505C544.574 146.005 520.074 114.505 410.074 173.005Z"
          fill="#E9562A"
        />
      </g>
      <defs>
        <filter
          id="filter0_f_153_132"
          x="0"
          y="-159"
          width="946.184"
          height="966.633"
        ...
        >
          ...
          <feGaussianBlur
            stdDeviation="150"
            result="effect1_foregroundBlur_153_132"
          />
        </filter>
      </defs>
    </svg>
    ...
    <BaseLayout>
      ...
      <div className="flex md:mt-0 md:mt-40 mt-16 flex-wrap justify-between md:container md:mx-auto md:px-0 px-4 md:mb-48 mb-24">
        {project?.imgList.map((img, index) => (
          <Image
            className={`md:mb-8 mb-4 object-cover ${
              [2, 3, 5, 6].includes(index)
                ? "md:w-[calc(50%_-_1rem)] w-full"
                : "w-full"
            }`}
            src={img}
            alt={img.src}
      ...
      <h3 className="md:container md:mx-auto md:px-0 px-4 text-white uppercase font-semibold text-2xl">
        Другие проекты
      </h3>

      <div className="md:container md:mx-auto md:px-0 px-4 justify-between flex flex-wrap pt-12">
        {getMultipleRandom(
          projectsList.filter((p) => p.slug !== router.query.slug),
          3
        ).map((project) => (
          <TeamMemberCard
            link={project.slug}
